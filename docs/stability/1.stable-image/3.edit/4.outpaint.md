...
The Outpaint service inserts additional content into an image to fill in empty space in any direction. Compared with other automated or manual expansion methods, it is designed to minimize artifacts and visible signs that the original image has been edited.
...
- image (required): the image you wish to outpaint

Along with at least one outpaint direction:
- left: number of pixels to outpaint on the left side
- right: number of pixels to outpaint on the right side
- up: number of pixels to outpaint on the top of the image
- down: number of pixels to outpaint on the bottom of the image

> Note: For best quality, use outpaint direction values smaller than or equal to your source image dimensions.
...
- creativity: controls the likelihood of creating additional details not heavily conditioned by the input image
...
A flat rate of 4 credits is charged for each successful generation. You will not be charged for failed generations.
...
- prompt: string [ 0 .. 10000 ] characters
...
### 200 Outpainting was successful
