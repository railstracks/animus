...
Inpaint intelligently modifies images by filling in or replacing specified areas with new content based on the contents of a mask image.

The mask can be provided in one of two ways:
- by explicitly passing a separate image via the mask parameter
- by deriving it from the alpha channel of the image parameter
...
Passing in a Mask:
- The image passed to this parameter should be a black-and-white image that represents the strength of inpainting at each pixel.
- Completely black pixels represent no inpainting strength, while completely white pixels represent maximum strength.
- If the mask is a different size than the image parameter, it will be automatically resized.

Alpha Channel Support:
- If you do not provide an explicit mask, one will be derived from the alpha channel of the image parameter.
- Transparent pixels will be inpainted, while opaque pixels will be preserved.
- If an image with an alpha channel is provided along with a mask, the mask will take precedence.

- grow_mask: number [ 0 .. 100 ]
-- Default: 5
