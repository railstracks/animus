...
Style Transfer applies visual characteristics from reference style images to target images. While Style Guide extracts stylistic elements from a control image to guide generation, Style Transfer specifically transforms existing content while preserving the original composition. This tool helps create consistent content across multiple assets.
...
- init_image (required): string <binary>
...
- style_image (required): string <binary>
...
-- Default: 1
*Sometimes referred to as denoising, this parameter controls how much influence the style_image parameter has on the generated image. A value of 0 would yield an image that is identical to the input. A value of 1 would be as if you passed in no image at all.*

- composition_fidelity: number [ 0 .. 1 ]
-- Default: 0.9
*How closely the output image's style resembles the input image's style.*

- change_strength: number [ 0.1 .. 1 ]
-- Default: 0.9
