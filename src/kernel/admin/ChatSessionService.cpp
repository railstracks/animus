    ...
    m_deps.jobs->EnqueueInLane(
        ...
        [wsConnPtr, session, sessionId, userContent, stopSignal,
         ...
         attachmentStore, attachmentTokenManager]() {
            ...
            if (agentConfig) {
                execReq.maxChainSteps = agentConfig->max_chain_steps;
                execReq.maxToolCallsPerChain = agentConfig->max_tool_calls_per_chain;
                execReq.timeoutSeconds = agentConfig->timeout_seconds;
            }
