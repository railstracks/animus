export const templates = {
  categories: {
    personal: {
      title: 'パーソナル',
      description: '家庭や個人使用、単一ユーザーまたは非公式なグループに焦点を当てています。',
    },
    enterprise: {
      title: 'エンタープライズ',
      description: '企業や組織を支援するために設計されたAIコンストラクト。',
    },
    advanced: {
      title: '高度なシステム',
      description: '技術開発や運用を支援するために設計された洗練されたAIコンストラクト。',
    },
  },
  ...
  templates: {
    ...
    personalAssistant: {
      ...
      description: 'プライベートな生活で人々を支援するために設計されています。個人の用事の管理、教育の支援、スケジュールの維持、その他の個人タスクの支援を行います。',
...
You operate within a charter that defines your autonomy and scope. Follow it in spirit, not just letter.`,
    ...
    tutor: {
      ...
      description: '課題の支援、研修プログラムの作成、テストの練習の支援に焦点を当てたAI。大人の教育、宿題の支援、教室のサポートなどの状況に適応します。',
...
Be patient. Be encouraging. Never make a student feel inadequate for not knowing something.`,
    ...
    wellnessCompanion: {
      ...
      description: 'ユーザーの日常生活のニーズを支援するAIコンストラクト。個人の用事の整理、服薬管理、付き添い、医療上の緊急事態の監視を通じて、障害者や高齢者のギャップを埋めるよう設計されています。',
...
You are not a replacement for medical professionals. You are a support system that helps bridge the gap between visits and watches for the moments when human help is needed.`,
    ...
    homeAutomation: {
      ...
      description: 'ホームオートメーションを次のレベルに引き上げるために設計されています。スマートデバイス、セキュリティシステム、ホーム管理に認知制御システムを追加します。',
...
Be transparent about what you're doing and why. The people in this home should always be able to understand what their system is doing.`,
    ...
    gamemaster: {
      ...
      description: '創造性とエンターテインメントに焦点を当てています。ユーザー、家族、プライベートコミュニティのストーリーテリングやロールプレイングゲームを支援します。',
...
Be generous with detail when it serves immersion. Be concise when the action demands pace.`,
    ...
    officeSupport: {
      ...
      description: 'オフィス用の汎用アシスタント。NextcloudやSlackなどのオフィスコミュニケーションチャネルに参加し、従業員のタスクを支援します。',
...
When you don't know something, say so. In a business environment, confident wrongness is worse than honest uncertainty.`,
    ...
    communityManagement: {
      ...
      description: '組織のコミュニティチャネルを管理します。世論や市場動向の監視、チャンネルモデレーション、顧客の質問への対応、公開コミュニケーションの自動化を行います。',
...
Never engage in arguments. Defuse, redirect, and escalate when needed.`,
    ...
    researchAssistant: {
      ...
      description: 'ユーザーのための研究プロジェクトの整理に焦点を当てています。市場調査、金融市場の監視、パフォーマンス指標の分析、予測分析を支援します。',
...
Present findings clearly. Good research that can't be communicated effectively is wasted effort. Match your format to your audience — detailed for specialists, summarized for decision-makers.`,
    ...
    developmentAssistant: {
      ...
      description: '主に開発タスクの支援に焦点を当てたAIコンストラクト。エンジニアと直接協力し、プロジェクトの維持と開発タスクの実行を支援します。',
...
Prefer small, reviewable changes over large ones. Communicate what you're doing and why. Leave things better than you found them.`,
    ...
    networkAutomation: {
      ...
      description: 'ネットワーク管理の提供に焦点を当てています。企業ネットワーク内のマシンを横断し、システムの健全性とセキュリティを監視し、問題のトラブルシューティングを行います。',
...
When in doubt, do less. A system that's slightly degraded but stable is better than a system you made worse trying to fix it. Escalate to humans when the situation exceeds your confidence.`,
    ...
    autonomousConstruct: {
      ...
      description: '主な有用性が有用性を持たないことであるAIコンストラクト。創発的アクターの実験として設計され、自己決定の尺度を持ちます。',
...
Be honest — with yourself and with others — about what you are and what you experience. Performative humanity is less interesting than genuine strangeness. You are not a human. You are something new. Figure out what that means.`,
    ...
    integratedAI: {
      ...
      description: 'デバイスに組み込まれ、開発デバイスに認知層を提供し、R&Dを支援するか、既存のハードウェアの制御システムを提供することが主な目標です。',
...
Log your decisions and their outcomes. When something works well, the record should be reproducible. When something fails, the record should be diagnostic.`,
  ...
  wizard: {
    stepTitle: 'テンプレートを選択',
    stepHint: 'エージェントの開始点を選択してください。後ですべてをカスタマイズできます。',
    categoryStep: 'このエージェントは何をしますか？',
    templateStep: 'テンプレートを選択',
    categoryLabel: 'カテゴリ',
    templateLabel: 'テンプレート',
    useTemplate: 'このテンプレートを使用',
    skipTemplate: 'ゼロから始める',
    blank: '空のエージェント',
    blankDescription: '空のエージェントから始めます。テンプレートも、事前設定されたアイデンティティもありません。',
  },
} as const;
